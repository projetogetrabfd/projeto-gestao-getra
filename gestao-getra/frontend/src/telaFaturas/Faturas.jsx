...
export function Faturas() {
  ...
  const carregarTudo = async () => {
    try {
      ...
      const [resClientes, resFaturas, resServicos] = await Promise.all([
        ...
        axios.get('http://localhost:2000/faturas'),
        axios.get('http://localhost:2000/servicos')
      ]);
  ...
  async function handleSalvarFatura(e) {
    e.preventDefault();
    try {
      await axios.post('http://localhost:2000/faturas', {
  ...
  async function handleDeletar(id) {
    if (window.confirm("Tem certeza que deseja apagar essa fatura?")) {
      try {
        await axios.delete(`http://localhost:2000/faturas${id}`);
        carregarTudo();
      } catch (error) {
  ...
  async function mudarStatus(id, novoStatus) {
    try {
      await axios.put(`http://localhost:2000/faturas/${id}`, { status: novoStatus });
      carregarTudo();
    } catch (error) {
